...
class MultiObjectiveFitness:
    ...
    def _energy_cost(self, instance, method_name: str, fn: Callable, trials: int = 2) -> float:
        """
        真实能耗近似评估：
        - 优先采用进程 CPU 时间差（user+system，总核心秒），更贴近“1 energy = 1 core-second”
        - 回退：wall time
        """
        def _call_once():
            ret = {'exc': None}
        ...
        try:
            if psutil is not None:
                ...
                c = proc.cpu_times()
                pre_cpu = float(getattr(c, 'user', 0.0) + getattr(c, 'system', 0.0))
        except Exception:
            proc = None
            pre_cpu = None
        ...
        for _ in range(trials):
            try:
                ...
                pass
        dt_wall = max(1e-6, time.perf_counter() - t0)

        post_cpu = None
        try:
            if proc is not None:
                c = proc.cpu_times()
                post_cpu = float(getattr(c, 'user', 0.0) + getattr(c, 'system', 0.0))
        except Exception:
            post_cpu = None
        ...
        if pre_cpu is not None and post_cpu is not None and post_cpu >= pre_cpu:
            d_cpu = max(1e-6, post_cpu - pre_cpu)
            return float(d_cpu * (1.0 + mem_diff / 1e5))
        else:
            return float(dt_wall * (1.0 + mem_diff / 1e5))
...
class DistributedLedger:
    ...
    # 新增：能量事件记录（mint/spend/transfer）
    def record_energy_event(self, node_id: str, kind: str, amount: float, metadata: Optional[Dict] = None):
        data = {
            'type': 'energy',
            'node_id': node_id,
            'kind': kind,  # 'mint' | 'spend' | 'transfer'
            'amount': float(amount),
            'timestamp': time.time(),
            'metadata': metadata or {}
        }
    ...
    # 新增：性繁殖与子代启动事件
    def record_sexual_reproduction(self, parent_a: str, parent_b: str, child_id: str, metadata: Optional[Dict] = None):
        data = {
    ...
    def record_child_launch(self, parent_id: str, child_id: str, cmdline: List[str], metadata: Optional[Dict] = None):
        data = {
            'type': 'spawn',
            'parent_id': parent_id,
            'child_id': child_id,
                ...
                for fn in os.listdir('chaindata'):
                    ...
                    for item in chain_data:
                        ...
                        if t in ('gene_transfer', 'announce', 'discovery'):
                            ...
                            if nid:
                                dead_nodes.add(nid)
                        elif t == 'sexual_reproduction':
                            for k in ('parent_a', 'parent_b', 'child_id'):
                                nid = data.get(k)
                                if nid:
                                    active_nodes.add(nid)
                        elif t == 'spawn':
                            nid = data.get('child_id')
                            if nid:
                                active_nodes.add(nid)
                        if t == 'announce':
                            nid = data.get('node_id')
...
# 新增：统一签名验签工具（供繁殖/语言/代码复用）
class CryptoUtils:
    @staticmethod
    def verify_signature(payload_b64: str, sig_hex: str, pubkey_hex: str) -> Tuple[bool, Optional[Dict]]:
        try:
            ...
            logger.debug(f"verify_signature failed: {e}")
            return False, None


# 能量管理器：按核心秒货币化能量（Windows 友好）
class EnergyManager:
    """
    令牌桶模型：
    - 补充速率 = quota_cores core-seconds / second
    - 消耗 = 进程 CPU 时间（user+system）核心秒
    - 1 energy = 1 core-second（可通过 energy_scale 调整）
    与 TrueDigitalLife.energy 双向对接（外部仍看到 [0, capacity]）
    """

    def __init__(self, owner, energy_scale: float = 1.0):
        self.owner = owner
        self.scale = float(energy_scale)
        self.capacity = float(owner.config.get('energy_capacity', 120.0))
        self._proc = None
        self._quota_cores = float(owner.config.get('cpu_quota_cores', 0.0)) or self._detect_quota_cores()
        self._last_wall = time.perf_counter()
        self._last_cpu = self._proc_cpu()
        self._record_enable = bool(owner.config.get('energy_record_enable', False))

    def _detect_quota_cores(self) -> float:
        cores = 1.0
        try:
            if psutil is not None:
                ...
                if hasattr(p, 'cpu_affinity'):
                    try:
                        cores = float(len(p.cpu_affinity()))
                        if cores >= 1.0:
                            return cores
                    except Exception:
                        pass
                # Windows 无 cpu_affinity 时退回逻辑核数
    ...
    def tick(self) -> Tuple[float, float, float]:
        """返回 (net, minted, spent)，单位 energy"""
        now = time.perf_counter()
        wall = max(0.0, now - self._last_wall)
        pre_cpu = self._last_cpu
        post_cpu = self._proc_cpu()
        d_cpu = max(0.0, post_cpu - pre_cpu)

        minted = self._quota_cores * wall * self.scale
        spent = d_cpu * self.scale
        net = minted - spent

        self._last_wall = now
        self._last_cpu = post_cpu
        return net, minted, spent

    def update(self):
        """将净变化应用到 owner.energy；按需记录链上事件"""
        try:
            net, minted, spent = self.tick()
            old = float(self.owner.energy)
            new = _clamp(old + net, 0.0, self.capacity)
            self.owner.energy = new
            if self._record_enable and hasattr(self.owner, 'blockchain'):
                if minted > 0.01:
                    self.owner.blockchain.record_energy_event(self.owner.node_id, 'mint', float(minted), {
                        'quota_cores': self._quota_cores, 'scale': self.scale
                    })
                if spent > 0.01:
                    self.owner.blockchain.record_energy_event(self.owner.node_id, 'spend', float(spent), {})
        except Exception as e:
            logger.debug(f"EnergyManager.update error: {e}")

    def require(self, cost: float, tag: str = 'op') -> bool:
        """尝试扣费；成功返回 True"""
        try:
            if float(self.owner.energy) >= float(cost):
                self.owner.energy = float(self.owner.energy) - float(cost)
                if self._record_enable and hasattr(self.owner, 'blockchain'):
                    self.owner.blockchain.record_energy_event(self.owner.node_id, 'spend', float(cost), {'tag': tag})
                return True
            return False
        except Exception:
            return False
    ...
    def budget(self, cost: float, tag: str = 'op'):
        ok = self.require(cost, tag=tag)
        try:
            yield ok
        finally:
            # 无自动退款逻辑；按需可扩展
...
class GeneticEncoder:
    ...
    def recombine_with_rate(self, dna1: str, dna2: str, mut_rate: float = 0.1) -> str:
        """重组两个DNA序列（带可调突变率）"""
        mut_rate = float(_clamp(mut_rate, 0.0, 0.5))
        new_dna = []
        for i in range(0, min(len(dna1), len(dna2)), 32):
            segment1 = dna1[i:i + 32]
            segment2 = dna2[i:i + 32]
            chosen = segment1 if random.random() < 0.5 else segment2
            if random.random() < mut_rate:
                chosen = hashlib.sha256(chosen.encode()).hexdigest()[:32]
            new_dna.append(chosen)
        return ''.join(new_dna)
...
class TrueDigitalLife:
    ...
    def __init__(self, genesis: bool = False, config: Optional[Dict] = None):
        ...
        self.config = {
            ...
            # 新增：能量货币化配置
            'energy_capacity': 120.0,         # 最大能量（单位 core-seconds）
            'energy_scale': 1.0,              # 1 energy = 1 core-second
            'energy_record_enable': True,     # 能量事件上链
            'cpu_quota_cores': 0.0,           # 留 0 表示自动探测
            'basal_burn_per_sec': 0.01,       # 基础代谢（每秒消耗 energy，受基因 metabolism 调制）

            # 新增：能量预算（进化/复制/交配）
            'evolution_energy_cost_base': 1.5,
            'replication_send_cost': 20.0,
            'birth_energy_cost': 60.0,
            'sexual_mate_min_energy': 50.0,

            # 繁殖认证开关（强制签名）
            'reproduction_signature_required': True,
        }
        if config:
        ...
        # 生命状态管理与身份
        # 允许父母为子代指定 node_id（用于链上 parent-child 关联）
        self.node_id = (self.config.get('node_id_override') or self._generate_node_id())
        ...
        # DNA 可从启动参数覆盖（父母传入）
        self.dna = self.config.get('dna_override') or self._generate_quantum_dna()
        # 基因 -> 行为参数 映射
        self._apply_gene_expression(self.genetic_encoder.decode(self.dna))
        ...
        # 能量系统：按核心秒货币化
        self.energy_manager = EnergyManager(self, energy_scale=self.config.get('energy_scale', 1.0))
        self.energy = float(self.config.get('energy_capacity', 120.0) * 0.5)  # 初始半容量
        self.metabolism = float(self.config.get('metabolism', 1.0))          # 作为基础代谢的调幅
        self._last_metabolism_ts = time.perf_counter()
        ...
        # 如果作为子代启动，记录父母信息到链上
        try:
            parents = self.config.get('parents')
            if parents and isinstance(parents, (list, tuple)) and len(parents) == 2:
                self.blockchain.record_sexual_reproduction(parents[0], parents[1], self.node_id, {
    ...
    def _apply_gene_expression(self, params: Dict):
        """将基因表达映射到配置与行为"""
        try:
            # 学习率
            lr = float(params.get('learning_rate', self.config.get('learning_rate', 0.001)))
            self.config['learning_rate'] = float(_clamp(lr, self.config.get('learning_rate_min', 1e-4), self.config.get('learning_rate_max', 2e-2)))
            # 变异率
            mr = float(params.get('mutation_rate', self.config.get('mutation_rate', 0.01)))
            self.config['mutation_rate'] = float(_clamp(mr, 0.005, 0.5))
            # 族群规模
            ps = int(params.get('pop_size', self.config.get('population_size', 8)))
            self.config['population_size'] = int(_clamp(ps, 4, 64))
            # 进化超时
            to = int(params.get('timeout', self.config.get('sandbox_timeout_ms', 800)))
            self.config['sandbox_timeout_ms'] = int(_clamp(to, 200, 3000))
            # 探索偏好 -> 影响代码进化概率
            exploration = float(_clamp(params.get('exploration', 0.3), 0.0, 1.0))
            base_evo = self.config.get('code_evolution_prob', 0.15)
            self.config['code_evolution_prob'] = float(_clamp(base_evo * (0.7 + 0.6 * exploration), 0.03, 0.5))
            # 新：基础代谢（每秒能量消耗）
            metab = float(_clamp(params.get('metabolism', 1.0), 0.5, 1.5))
            self.config['basal_burn_per_sec'] = float(_clamp(0.008 * metab, 0.001, 0.05))
            self.config['metabolism'] = metab
            # 运算符偏置 -> 鼓励新函数/复制相关
            op_bias = float(_clamp(params.get('op_bias', 0.5), 0.0, 1.0))
            if hasattr(self, 'code_engine'):
                for k in list(self.code_engine.operator_weights.keys()):
                    if k in ('synth_local', 'synth_inst', 'inject_call_any', 'replication'):
                        self.code_engine.operator_weights[k] *= (0.8 + 0.8 * op_bias)
                # 同步变异率
                self.code_engine.adaptive_mutation_rate = float(_clamp(self.config['mutation_rate'], 0.02, 0.5))
            # 神经模型学习率同步
            try:
                for m in self.neural_net.get('models', {}).values():
                    if hasattr(m, 'learning_rate_init'):
                        m.learning_rate_init = self.config['learning_rate']
            except Exception:
                pass
        except Exception as e:
            logger.debug(f"apply_gene_expression failed: {e}")
    ...
    def _verify_and_register_sender(self, pubkey_hex: str, source_node: str, host: Optional[str], port: Optional[int]) -> bool:
        """校验发件人的公钥是否与链上公告一致；若未知则自动登记"""
        try:
            addr_map = self.blockchain.get_node_address_map()
            if source_node not in addr_map:
                if host and isinstance(port, int) and 1 <= port <= 65535:
                    if self._is_allowed_destination(host):
                        self.blockchain.record_announce(source_node, host, port, pubkey_hex)
                        return True
                return False
            else:
                _, _, announced_pub = addr_map[source_node]
                return not announced_pub or (announced_pub == pubkey_hex)
        except Exception:
            return False
    ...
    def _init_api(self):
        ...
        def receive_code_signed():
            if self.config.get('allowlist') and request.remote_addr not in self.config.get('allowlist', []):
                return jsonify({'status': 'forbidden'}), 403
            if self.state == LifeState.REPLICATING:
                return jsonify({'status': 'busy_replicating'}), 400
            ...
            try:
                ok, data = CryptoUtils.verify_signature(code_data['payload'], sig_hex, pubkey_hex)
                if not ok:
                ...
                if not self._verify_and_register_sender(pubkey_hex, source_node, host, port):
                    return jsonify({'status': 'unknown_sender'}), 403
            except Exception:
                pass
        ...
        def speak_signed():
            ...
            try:
                ok, data = CryptoUtils.verify_signature(pkt['payload'], sig_hex, pub_hex)
                if not ok:
                    return jsonify({'status': 'verify_failed'}), 400
            except Exception as e:
            ...
            try:
                ...
                if source_node and source_node not in addr_map:
                    ...
                    if 1 <= port <= 65535 and self._is_allowed_destination(host):
        ...
        # ========= 性繁殖：签名校验 + 接收方启动子进程 =========
        @self.api.route('/mate/propose', methods=['POST'])
        def mate_propose():
            if not self.config.get('network_enable', True):
                return jsonify({'status': 'network_disabled'}), 403
            pkt = request.json or {}
            if not pkt or 'payload' not in pkt or 'sig' not in pkt or 'pubkey' not in pkt:
                return jsonify({'status': 'invalid'}), 400
            if self.config.get('reproduction_signature_required', True):
                ok, data = CryptoUtils.verify_signature(pkt['payload'], pkt['sig'], pkt['pubkey'])
                if not ok:
                    return jsonify({'status': 'verify_failed'}), 400
            else:
                try:
                    data = json.loads(base64.b64decode(pkt['payload']).decode('utf-8'))
                except Exception:
                    return jsonify({'status': 'bad_payload'}), 400

            meta = data.get('meta', {})
            parent_a = str(meta.get('source_node', ''))
            donor_dna = data.get('dna', '')
            host = meta.get('host') or request.remote_addr
            try:
                port = int(meta.get('port', 0))
            except Exception:
                port = 0
            # 校验来源公钥与公告一致
            if not self._verify_and_register_sender(pkt['pubkey'], parent_a, host, port):
                return jsonify({'status': 'unknown_sender'}), 403

            # 能量与状态判定
            if self.state == LifeState.REPLICATING:
                return jsonify({'status': 'busy'}), 429
            if self.energy < self.config.get('sexual_mate_min_energy', 50.0):
                return jsonify({'status': 'low_energy'}), 400
            if not self._validate_dna(donor_dna):
                return jsonify({'status': 'invalid_dna'}), 400

            # 交叉+突变率 = 双方 mutation_rate 的平均
            try:
                pA = self.genetic_encoder.decode(donor_dna)
                pB = self.genetic_encoder.decode(self.dna)
                mut_rate = float(_clamp(0.5 * (pA.get('mutation_rate', 0.05) + pB.get('mutation_rate', 0.05)), 0.01, 0.2))
                child_dna = self.genetic_encoder.recombine_with_rate(self.dna, donor_dna, mut_rate=mut_rate)
            except Exception:
                child_dna = self.genetic_encoder.recombine(self.dna, donor_dna)

            # 启动子进程（接收方为 parent_b）
            with self.energy_manager.budget(self.config.get('birth_energy_cost', 60.0), tag='birth') as ok:
                if not ok:
                    return jsonify({'status': 'insufficient_energy'}), 402
                child_id = self._spawn_child_process(child_dna, parent_a=parent_a, parent_b=self.node_id)
                self.blockchain.record_sexual_reproduction(parent_a, self.node_id, child_id, {
                    'mut_rate': mut_rate, 'host': self.config.get('host'), 'port': self.config.get('port')
                })
                return jsonify({'status': 'accepted', 'child_id': child_id})

        @self.api.route('/mate/accept', methods=['POST'])
        def mate_accept():
            """
            预留的第二阶段接口（当前流程由 /mate/propose 即时完成并启动子进程）。
            这里仅校验签名并回执接受状态。
            """
            pkt = request.json or {}
            if not pkt or 'payload' not in pkt or 'sig' not in pkt or 'pubkey' not in pkt:
            ...
            if not ok:
                return jsonify({'status': 'verify_failed'}), 400
            return jsonify({'status': 'not_required', 'note': 'spawn happens at /mate/propose receiver'})
    ...
    def _metabolism_cycle(self):
        """代谢循环 - 真实能量模型（核心秒）+ 基础代谢"""
        self.age += 1
        # 基于进程 CPU 时间差增发/扣费
        try:
            self.energy_manager.update()
        except Exception:
            pass

        # 基因驱动的基础代谢（按 wall-time）
        try:
            now = time.perf_counter()
            dt = max(0.0, now - self._last_metabolism_ts)
            basal = float(self.config.get('basal_burn_per_sec', 0.01)) * float(self.config.get('metabolism', 1.0)) * dt
            if basal > 0.0:
                self.energy = max(0.0, self.energy - basal)
            self._last_metabolism_ts = now
        except Exception:
            pass
    ...
    def _environment_scan(self):
        ...
        with self._mem_lock:
            self.short_term_memory.append({
                ...
                'energy': self.energy,
                'pleasure': self.pleasure if hasattr(self, 'pleasure') else 0.5,
                'stress': self.stress if hasattr(self, 'stress') else 0.2
            })
    ...
    def _survival_goal_evaluation(self, update_state: bool = True):
        """生存目标评估系统"""
        energy_goal = min(1.0, self.energy / max(1.0, self.config.get('energy_capacity', 120.0)))
        ...
        if update_state:
            self.pleasure = min(1.0, max(0, getattr(self, 'pleasure', 0.5) + (survival_score - 0.5) * 0.1))
            self.stress = min(1.0, max(0, getattr(self, 'stress', 0.2) + (1 - survival_score) * 0.1))
    ...
    def _memory_consolidation(self):
        ...
        try:
            ...
            for mem in recent_memories:
                features.append([
                    mem['energy'] / max(1.0, self.config.get('energy_capacity', 120.0)),
    ...
    def _evolution_cycle(self):
        ...
        # 能量预算检查（与族群规模相关）
        evo_cost = float(self.config.get('evolution_energy_cost_base', 1.5)) * float(self.config.get('population_size', 8))
        ok_budget = self.energy_manager.require(evo_cost, tag='evolution')
        if not ok_budget:
            return
    ...
    def _code_replication(self):
        ...
        # 能量预算
        if not self.energy_manager.require(self.config.get('replication_send_cost', 20.0), tag='replicate'):
            return
        ...
        try:
            ...
            if success:
                ...
                self.pleasure = min(1.0, getattr(self, 'pleasure', 0.5) + 0.2)
            else:
                logger.warning("Code replication failed on all targets")
                self.stress = min(1.0, getattr(self, 'stress', 0.2) + 0.1)

        except Exception as e:
            logger.error(f"Replication error: {e}")
            self.stress = min(1.0, getattr(self, 'stress', 0.2) + 0.15)
        finally:
            with self._lock:
                self.state = LifeState.ACTIVE
    ...
    def _create_replication_package(self) -> Dict:
        ...
        safe_config = {k: v for k, v in self.config.items() if k not in ('auth_token', 'allowlist', 'node_id_override')}
    ...
    # ======= 子进程启动（Windows 友好） =======
    def _spawn_child_process(self, child_dna: str, parent_a: str, parent_b: str) -> str:
        try:
            child_id = hashlib.sha3_256(f"{self.node_id}-{time.time_ns()}-{uuid.uuid4().hex}".encode()).hexdigest()[:32]
            exe = sys.executable or 'python'
            script = Path(sys.argv[0]).resolve()
            if not script.exists() and '__file__' in globals():
            ...
            if platform.system().lower().startswith('win'):
                try:
                    ...
                    creationflags = 0
            p = subprocess.Popen(args, close_fds=False, creationflags=creationflags)
            self.blockchain.record_child_launch(self.node_id, child_id, args, {'pid': p.pid})
            logger.info(f"Spawned child {child_id} with PID {p.pid}")
            return child_id
        except Exception as e:
            logger.error(f"spawn child failed: {e}")
            return 'unknown'
...
if __name__ == "__main__":
    ...
    while i < len(argv):
        ...
        if arg == "--genesis":
            genesis = True
            i += 1
        elif arg == "--dna" and i + 1 < len(argv):
            dna_override = argv[i + 1]
            i += 2
        elif arg == "--parents" and i + 1 < len(argv):
            pair = argv[i + 1]
            if ',' in pair:
                pa, pb = pair.split(',', 1)
                parents = [pa.strip(), pb.strip()]
            i += 2
        elif arg == "--node-id" and i + 1 < len(argv):
            node_id_override = argv[i + 1].strip()
            i += 2
        else:
            i += 1

    # 初始化数字生命（支持 DNA/父母/子代 ID 覆盖）
    cfg = {}
    if dna_override:
        cfg['dna_override'] = dna_override
    if parents:
        cfg['parents'] = parents
    if node_id_override:
        cfg['node_id_override'] = node_id_override

    life = TrueDigitalLife(genesis=genesis, config=cfg)
